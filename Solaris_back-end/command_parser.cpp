...
QByteArray command_parser::command(QString cmd)
{
    ...
    if (shortList.first()=="start")
    {
        ...
        {
               bytes.append("Старт вычеслений солнца\r\n");
               module->start_sun();
        }
        else
        {
    ...
    else if (shortList.first()=="stop")
    {
        ...
        {
            bytes.append("Остановка вычисления  потока солнца\r\n");
            module->stop_sun();
        }
        else
        {
    ...
    } else if (shortList.first()=="touch")
    {    if (shortList.size()>2){
        if (shortList[1]=="salleter")
        {
            if (shortList[2].toInt()){
            bytes.append("Установлено количество("+shortList[2]+") итераций для вычислния E\r\n");
            module->touch_salleter(shortList[2].toInt());
            }
            else{
                bytes.append("веддите число\r\n");
            }
        }
        else
        {
    ...
    } else if (shortList.first() == "get")
    {
        ...
        else if (shortList[1]=="earth_pos")
        {
            sun earth;
            earth=module->get_earth_pos();
            QString x,y,z;
            x.setNum(earth.x);
            y.setNum(earth.y);
            z.setNum(earth.z);
            bytes.append("Положение земли x-|"+x+"|, y-|"+y+"| z-|"+z+"|\r\n");
        }
    ...
    else if (shortList.first()=="help")
    {
        ...
        bytes.append("replay san <int> - Установка повтора вычислений для потока солнца\n\r");
        bytes.append("touch salleter <int> - Установка количество итераций для вычисления E\n\r");
        ...
        bytes.append("debug module_modul <true/false> - Вывод  в терменал дополнительных данных из module_module\n\r");
        ...
        bytes.append("get earth_pos -Возращает местоположение земли в гелиоцентрической системе координат (VSOP87)\n\r");
        bytes.append("get value_corners_salleter <name>-Возращает местоположение спутника\n\r");
        bytes.append("new salleter<name,time_ut,a,e,i,ark_per,dolgot,m>-добавление нового спутника в бд и запуск расчетов\n\r");
        bytes.append("new user <login,password>-добавление нового пользователя в бд для его авторизаии\n\r");
